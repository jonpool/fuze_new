	...
	return (
		<>
			...
			<ul className="space-y-16">
				...
				<li>
					Simplify the logic with CSS variables, removing the unnecessary <code>item</code> prop and reducing
					CSS specificity.
				</li>
				<li>
					Introduce a proper fix for <a href="#prevent-scrollbar">preventing a scrollbar</a> by switching
					between negative margin approaches.
				</li>
			...
			>
				...
				<code>Unstable_Grid2</code> to gather feedbacks from the community before making it stable in the next
...
`}
			...
			>
				...
				<a href="#multiple-breakpoints">Multiple breakpoints</a> for details. For example, a component with{' '}
			...
			>
				...
				number, then a column&#39;s size will automatically adjust to match the width of its content. The demo
...
`}
			...
			>
				...
				the grid. These props accept:
			</Typography>
			<ul className="space-y-16">
				<li>
					numbers—for example, <code>{`mdOffset={2}`}</code> pushes an item two columns to the right when the
					viewport size is equal to or greater than the <code>md</code> breakpoint.
				</li>
				<li>
...
`}
			...
			>
				...
				<code>true</code> will appear as <code>{`{key}`}</code>(size prop) and <code>{`{key}Offset`}</code>
				(offset prop).
			</Typography>
...
`}
			...
			>
				...
				<code>disableEqualOverflow</code> is <code>true</code> because the negative margin (applied only at the
			...
			>
				...
				within containers that use <code>{`direction="column"`}</code> or{' '}
