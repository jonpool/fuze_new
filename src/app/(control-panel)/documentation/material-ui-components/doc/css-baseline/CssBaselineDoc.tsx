	...
	return (
		<>
			...
			>
				...
				collection of HTML element and attribute style-normalizations.
			</Typography>
...
`}
			...
			>
				...
				be an option. It&#39;s possible to apply the baseline only to the children by using the{' '}
...
`}
			...
			<ul className="space-y-16">
				<li>
					...
					are declared to inherit this property, which ensures that the declared width of the element is never
			...
			>
				...
				code to your theme (for dark mode).
			</Typography>
...
`}
			...
			>
				...
				<code>{`"dark"`}</code> modes of native components such as scrollbar, using the{' '}
				<code>color-scheme</code> CSS property. To enable it, you can set <code>enableColorScheme=true</code> as
				follows:
			</Typography>
...
`}
			...
			<ul className="space-y-16">
				<li>
					...
					font size in{' '}
					<a href="/material-ui/customization/typography/#html-font-size">the theme documentation</a> page.
				</li>
				<li>
					Set the <code>theme.typography.body1</code> style on the <code>{`<body>`}</code> element.
				</li>
				<li>
					Set the font-weight to <code>theme.typography.fontWeightBold</code> for the <code>{`<b>`}</code> and{' '}
			...
			>
				Head to the{' '}
				<a href="/material-ui/customization/how-to-customize/#4-global-css-override">global customization</a>{' '}
